...
const segmentDensitySchema = mongoose.Schema({
...
const segmentDensityStorage = mongoose.model('dataDriftSegmentTrain', segmentDensitySchema, _get(CONSTANTS, 'collectionName.dataDriftSegmentTrain'));
const  segmentDensityTrainSegmentFilter= (field,getFields) => segmentDensityStorage.distinct(field,getFields);



// const filedMetricFindOne = getFields => FiledMetricStorage.findOne(getFields);
const segmentDensityFind = getFields => segmentDensityStorage.find(getFields);
const segmentDensityFindWithOptions = ({ query, field, options }) => segmentDensityStorage.find(query, field, options);
...
module.exports = {
    segmentDensityFind,
    segmentDensityFindWithOptions,
    segmentDensityTrainSegmentFilter
